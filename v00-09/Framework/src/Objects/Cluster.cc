...
{
    //std::cout<< "Inside Cluster::CalculateFastPhotonFlag 1 " << std::endl;
    const bool fastPhotonFlag(ParticleIdHelper::IsPhotonFast(this));

    //std::cout<< "Inside Cluster::CalculateFastPhotonFlag 2 " << std::endl;
...
{
    //std::cout << "Inside Cluster::CalculateLayerHitType 1 " << std::endl;
    ...
    for (CaloHitList::const_iterator iter = listIter->second->begin(), iterEnd = listIter->second->end(); iter != iterEnd; ++iter)
    {
        ...
        {   
            //NS Fix
            mapIter->second += (*iter)->GetHadronicEnergy() + 0.00001;
            continue;
        }
        // Same as above from NS
        if (!hitTypeToEnergyMap.insert(HitTypeToEnergyMap::value_type((*iter)->GetHitType(), (*iter)->GetHadronicEnergy()  + 0.00001  )).second )
//        if (!hitTypeToEnergyMap.insert(HitTypeToEnergyMap::value_type((*iter)->GetHitType(), (*iter)->GetHadronicEnergy() )).second )
            throw StatusCodeException(STATUS_CODE_FAILURE);
    }

    float highestEnergy(0.00001);
//    float highestEnergy(0.f);
    ...
    {
        //std::cout << "Inside Cluster::CalculateLayerHitType 2 HitTypeToEnergyMap highest energy " << iter->second  << std::endl;
        if (iter->second > highestEnergy)
        {
            layerHitType = iter->first;
            highestEnergy = iter->second; 
        } 
        //std::cout << "Inside Cluster::CalculateLayerHitType 3 HitTypeToEnergyMap highest energy " << highestEnergy   << std::endl;
