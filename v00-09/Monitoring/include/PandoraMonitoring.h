    ...
    m_energyScaleThresholdE(-1.f)
{
    ...
    //char**      argv = const_cast<char**>(dummyArgvArray);
    //.gROOT->SetBatch(kFALSE);
    //TApplication::NeedGraphicsLibs();
    std::cout << TApplication::GetApplications()->GetSize() << std::endl;
    std::cout << "In PandoraMonitoring::PandoraMonitoring 1 "  << std::endl;
    ...
    std::cout << "In PandoraMonitoring::PandoraMonitoring gApplication " << gApplication << std::endl; 
    //std::cout << "In PandoraMonitoring::PandoraMonitoring gApplication->TestBit(kDefaultApplication) " << gApplication->TestBit(kDefaultApplication) << std::endl;
    //std::cout << "In PandoraMonitoring::PandoraMonitoring  " <<  << std::endl;
    // allow default TApplication to be replaced by a "real" TApplication
    std::cout << "In PandoraMonitoring::PandoraMonitoring gVirtualX->GetDisplay()" << gVirtualX->GetDisplay() << std::endl;
    //TGLWidget* w = TGLWidget::Create(gClient->GetRoot(), kTRUE, kTRUE, 0, 10, 10);
    //std::cout << "In PandoraMonitoring::PandoraMonitoring gVirtualX->GetDisplay() after TGLWidget " << gVirtualX->GetDisplay() << std::endl;
    //delete gApplication;
    //gApplication = 0;
    //gROOT->SetBatch(kFALSE);
    //TApplication::NeedGraphicsLibs();
    //gApplication->SetBit(1);

    if(!m_pApplication){  
       std::cout << "In if of m_pApplication " << std::endl;   
       m_pApplication = new TApplication("PandoraMonitoring", &argc, &argv);
    }
    std::cout << "m_pApplication->ApplicationName()" << m_pApplication->ApplicationName() << std::endl;

    std::cout << "In PandoraMonitoring::PandoraMonitoring 2 "  << std::endl;
